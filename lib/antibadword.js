...
async function handleBadwordDetection(sock, chatId, message, userMessage, senderId) {
    // Skiasync function handleBadwordDetection(sock, chatId, message, userMessage, senderId) {
    console.log('[DEBUG] 1. Function called');
    console.log('[DEBUG] 2. Chat ID:', chatId);
    console.log('[DEBUG] 3. Message:', userMessage);
    
    // Skip if not group
    if (!chatId.endsWith('@g.us')) return;
    console.log('[DEBUG] 4. Is group: YES');
    
    // Skip if message from bot
    if (message.key.fromMe) return;
    console.log('[DEBUG] 5. Not from bot');
    
    // Get antibadword config
    const config = await getAntiBadword(chatId, 'on');
    console.log('[DEBUG] 6. Config:', JSON.stringify(config));
    
    // Check if enabled
    if (!config || !config.enabled) {
        console.log('[DEBUG] 7. NOT enabled - exiting');
        return;
    }
    console.log('[DEBUG] 8. IS enabled');
    
    // Load bad words
    const badWords = loadBadWords(chatId);
    console.log('[DEBUG] 9. Bad words:', badWords);
    
    if (badWords.length === 0) {
        console.log('[DEBUG] 10. No bad words - exiting');
        return;
    }
    
    // Rest of your existing code continues here...p if not group
    if (!chatId.endsWith('@g.us')) return;
